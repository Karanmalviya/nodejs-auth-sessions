...
export default function Sidebar() {
  ...
  return (
    <div className="flex flex-col py-8 pl-6 pr-2 w-64 bg-white flex-shrink-0">
      ...
      <div className="flex flex-row items-center bg-indigo-100 border border-gray-200 mt-4 w-full py-3 px-4 rounded-lg space-x-3">
        <div className="flex h-12 w-12 rounded-full border overflow-hidden">
          <img
        ...
        <div className="flex flex-col space-y-1 mt-4 -mx-2 h-48 overflow-y-auto">
          {users?.length > 0 &&
            users.map((user, index) => (
              <button
                key={index}
                className={`flex flex-row items-center rounded-xl p-2 cursor-pointer ${
                  user?._id === id ? "bg-gray-100" : "hover:bg-gray-100"
                }`}
                onClick={() => {
                  navigate("/" + user?._id);
                }}
